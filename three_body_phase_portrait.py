...
class Threebody:
	...
	def accelerations(self, p1, p2, p3):
		...
		Args:
			p1: np.ndarray(np.meshgrid[float])
			p2: np.ndarray(np.meshgrid[float])
			p3: np.ndarray(np.meshgrid[float])

		Return:
			planet_1_dv: np.ndarray(np.meshgrid[float])
			planet_2_dv: np.ndarray(np.meshgrid[float])
			planet_3_dv: np.ndarray(np.meshgrid[float])
	...
	def sensitivity(self, y_res, x_res, steps):
		...
		y, x = np.arange(-20, 20, 0.1), np.arange(-20, 20, 0.1)
		grid = np.meshgrid(x, y)
		# grid of all -11, identical starting z-values
		z = np.zeros(grid[0].shape) - 11

		# shift the grid by a factor of 1e-4 
		grid2 = grid[0] + grid[0]/1e2, grid[1] + grid[1]/1e2
		# grid of all -11, identical starting z-values
		z_prime = np.zeros(grid2[0].shape) - 11
		...
		# p1_start = x_1, y_1, z_1
		p1_prime = np.array([grid2[0], grid2[1], z])
		...
		# assign masses of planets
		m_1, m2, m3 = 10, 20, 30

		# bool array of all false
		not_still_together = grid[0] > 1e10

		# evolution of the system
		for i in range(self.time_steps):
			print (i)
			diverging = self.diverged(p1, p1_prime)

			# points still together are not diverging now and have not previously
			not_still_together &= diverging

			# apply boolean mask to ndarray time_array
			time_array[~not_still_together] += 1
			...
			v1_prime = v1_prime + dv1_prime * delta_t
			v2_prime = v2_prime + dv2_prime * delta_t
			v3_prime = v3_prime + dv3_prime * delta_t
